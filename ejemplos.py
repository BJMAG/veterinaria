...
class AdminPanel(QWidget):
    def __init__(self, parent_login):
        super().__init__()
        self.parent_login = parent_login  # Guardamos la referencia al login
    ...
    def closeEvent(self, event):
        """Cuando se cierre el panel, mostramos el login de nuevo"""
...
class AuxPanel(QWidget):
    def __init__(self, role, parent_login):
        super().__init__()
        self.parent_login = parent_login  # Guardamos la referencia al login
    ...
    def aceptar(self):
        ...
        if pins.get(self.role) == pin:
            ...
            if self.role == "Administrador":
                self.panel = AdminPanel(self.parent())
            else:
                self.panel = AuxPanel(self.role, self.parent())
